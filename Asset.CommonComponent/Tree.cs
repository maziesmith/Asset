	...
	public class Tree
	{
		...
		private void CreateSubTree(int nodeId)
		{
			...
			foreach(DataRow dr in childNodes.Rows)
			{
				...
				{
                    this._treeHtml += "<img src='.\\Images\\folder.gif'/><a href=" + dr["Url"] + " target=" + dr["UrlTarget"] + " style='text-decoration:none;'>" + dr["Text"] + "</a></div>";
                    //this._treeHtml += "<a href=" + dr["Url"] + " target=" + dr["UrlTarget"] + ">" + dr["Text"] + "</a></div>";
				}
				//如果该孩子为中间节点，则首先构造其HTML，然后递归生成其所有孩子的HTML
				else
				{
					this._treeHtml+="<img src='.\\Images\\folderopen.gif'/><a href="+dr["Url"]+" style='text-decoration:none;font-weight:bold;color:#000000;'>"+dr["Text"]+"</a></div>";
